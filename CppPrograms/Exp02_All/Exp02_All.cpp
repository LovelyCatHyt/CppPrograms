...
float GetCost(float consumePerOne)
{
	//一行处理一个区间
	if (consumePerOne < 20) return consumePerOne * 0.6;
	if (consumePerOne < 30) return consumePerOne - 8;//12+(x-20)=x-8
	return 2 * consumePerOne - 38;//22+2(x-30)=2x-38
}
...
int main()
{
	...
	int count = 1;
	float temp = 0;
	cout << "Please enter total consume: " << endl<<">";
	cin >> consume;
	cout << "Please enter the number of people:" << endl<<">";
	cin >> count;
	temp = GetCost(consume / count);
	cout << "Cost per people is: " << temp << endl;
	cout << "Total cost: " << temp * count << endl;
}
